...
class RouterModel20B:
    ...
    def route_query(self, query: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        Rutea una query a través del sistema de routing semántico.
        ...
        Returns:
            Dict con la decisión de routing
        """
        if context is None:
            context = {}
        
        try:
            # Decidir si escalar (complejidad alta o baja confianza de dominio)
            should_escalate = self.should_escalate(query, context)
            
            # Calcular complejidad y confianza
            complexity = self._calculate_complexity(query)
            domain_confidence = self._calculate_domain_confidence(query, context)
            
            # Determinar modelo recomendado basado en la complejidad
            # Orden de modelos de mayor a menor complejidad: gpt-oss:20b, mistral, phi3:mini
            if complexity > 0.7:  # Alta complejidad
                recommended_model = "gpt-oss:20b"
                model_tier = "complex"
            elif complexity > 0.4:  # Media complejidad
            ...
            # Detectar si la query implica generación de código
            code_related = self._is_code_related(query)
            
            # Seleccionar template E2B apropiado basado en el contenido
            e2b_template = self._select_e2b_template(query, code_related)
            ...
            routing_result = {
                ...
                'e2b_template_suggestion': e2b_template,
                'routing_timestamp': datetime.now().isoformat(),
                'reasoning': self._generate_routing_reasoning(complexity, domain_confidence, should_escalate, code_related)
            }
            
            logger.info(f"Query ruteada: {routing_result['recommended_model']} ({model_tier}), "
                       f"complejidad={complexity:.2f}, código={code_related}")
            
            return routing_result
            
        except Exception as e:
            logger.error(f"Error en route_query: {e}")
            # En caso de error, retornar decisión por defecto
            return {
                'query_id': f"query_{hash(query) % 10000}",
                'recommended_model': "phi3:mini",
                'model_tier': 'fast_response',
                ...
                'code_related': False,
                'e2b_template_suggestion': 'default',
                'routing_timestamp': datetime.now().isoformat(),
                'reasoning': 'Error en análisis, usando modelo por defecto'
            }

    def _is_code_related(self, query: str) -> bool:
        """Detecta si la query está relacionada con generación de código."""
        code_indicators = [
            'code', 'codigo', 'program', 'script', 'function', 'definir', 'implementar',
            'crear', 'generar', 'ejecutar', 'python', 'javascript', 'sql', 'código',
            'función', 'script', 'algoritmo', 'programar', 'desarrollar', 'hacer',
            'ejemplo de', 'muestra de código', 'escribe un', 'show me', 'como hacer',
            'como crear', 'implementación', 'librería', 'framework', 'api'
        ]
        
        query_lower = query.lower()
        return any(indicator in query_lower for indicator in code_indicators)

    def _select_e2b_template(self, query: str, is_code_related: bool) -> str:
        """Selecciona el template E2B más apropiado basado en la query."""
        ...
        if any(keyword in query_lower for keyword in ['data', 'pandas', 'analysis', 'análisis', 'dataset', 'csv', 'excel', 'tabla']):
            return 'data_analysis'
        elif any(keyword in query_lower for keyword in ['visual', 'chart', 'grafic', 'plot', 'gráfico', 'gráfica', 'matplotlib', 'seaborn']):
            return 'visualization'
        elif any(keyword in query_lower for keyword in ['ml', 'machine learning', 'ai', 'inteligencia', 'aprendizaje', 'modelo', 'model']):
            return 'machine_learning'
        elif any(keyword in query_lower for keyword in ['quick', 'simple', 'pequeño', 'básico', 'fácil']):
            return 'quick_script'
        else:
            return 'default'

    def _generate_routing_reasoning(self, complexity: float, domain_confidence: float,
                                   should_escalate: bool, code_related: bool) -> str:
        """Genera razonamiento para la decisión de routing."""
        reasons = []
        
        if complexity > 0.7:
            reasons.append(f"Alta complejidad detectada ({complexity:.2f} > 0.7)")
            reasons.append("Recomendando modelo gpt-oss:20b (complex tier)")
        elif complexity > 0.4:
            reasons.append(f"Complejidad media detectada ({complexity:.2f} entre 0.4 y 0.7)")
            reasons.append("Recomendando modelo mistral (balanced tier)")
        else:
            reasons.append(f"Baja complejidad detectada ({complexity:.2f} <= 0.4)")
            reasons.append("Recomendando modelo phi3:mini (fast_response tier)")
            
        if domain_confidence < self.domain_confidence_threshold:
            reasons.append(f"Baja confianza de dominio ({domain_confidence:.2f} < {self.domain_confidence_threshold})")
        else:
            reasons.append(f"Confianza de dominio adecuada ({domain_confidence:.2f} >= {self.domain_confidence_threshold})")
        
        if code_related:
            reasons.append("Relacionado con generación de código")
