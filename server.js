// server.js - Servidor Express para probar el sistema CTM
const express = require('express');
const app = express();

// Cargar configuración
const modelConfig = require('./model_config.json');
const { VLLMClient } = require('./ollama_client.js');  // Keep same filename for compatibility
const { TaskClassifier } = require('./task_classifier.js');

const vllmClient = new VLLMClient(modelConfig);
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`Servidor CTM corriendo en el puerto ${PORT}`);
  console.log('Endpoints disponibles:');
  console.log('  POST /api/ai/generate - Generar texto con clasificación automática');
  console.log('  POST /api/ai/classify - Clasificar tarea sin ejecutarla');
  console.log('  POST /api/ai/:modelTier/generate - Generar texto con modelo específico');
});